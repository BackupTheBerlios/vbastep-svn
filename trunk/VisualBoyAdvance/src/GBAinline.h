...
inline u32 CPUReadMemory(u32 address)
{
  ...
  if(address & 3) {  
    if(systemVerbose & VERBOSE_UNALIGNED_MEMORY) {
      emulog("Unaligned word read: %08x at %08x\n", address, armMode ?
    ...
    if(reg[15].I >> 24) {
      if(address < 0x4000) {
        ...
        if(systemVerbose & VERBOSE_ILLEGAL_READ) {
          emulog("Illegal word read: %08x at %08x\n", address, armMode ?
              armNextPC - 4 : armNextPC - 2);
        }
#endif
...
inline u32 CPUReadHalfWord(u32 address)
{
  ...
  if(address & 1) {
    if(systemVerbose & VERBOSE_UNALIGNED_MEMORY) {
      emulog("Unaligned halfword read: %08x at %08x\n", address, armMode ?
          armNextPC - 4 : armNextPC - 2);
    }
  }
#endif
...
#ifdef DEV_VERSION
        if(systemVerbose & VERBOSE_ILLEGAL_READ) {
          emulog("Illegal halfword read: %08x at %08x\n", address, armMode ?
              armNextPC - 4 : armNextPC - 2);
        }
#endif
...
inline void CPUWriteMemory(u32 address, u32 value)
{
  ...
  if(address & 3) {
    if(systemVerbose & VERBOSE_UNALIGNED_MEMORY) {
      emulog("Unaliagned word write: %08x to %08x from %08x\n",
          value,
          address,
  ...
  unwritable:
    ...
    if(systemVerbose & VERBOSE_ILLEGAL_WRITE) {
      emulog("Illegal word write: %08x to %08x from %08x\n",
          value,
          address,
          armMode ? armNextPC - 4 : armNextPC - 2);
    }
