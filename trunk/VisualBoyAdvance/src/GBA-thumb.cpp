...
static INSN_REGPARM void thumbB4(u32 opcode)
{
  ...
  PUSH_REG(64, 6);
  PUSH_REG(128, 7);
  clockTicks = 1 + count + dataTicksAccessMulti32(address,count)
...
static INSN_REGPARM void thumbBC(u32 opcode)
{
  ...
  POP_REG(128, 7);
  reg[13].I = temp;
  clockTicks = 2 + count + dataTicksAccessMulti32(address,count)
      + codeTicksAccess16(armNextPC);
}
...
static INSN_REGPARM void thumbBD(u32 opcode)
{
  ...
  count++;
  clockTicks = 3 + count + dataTicksAccessMulti32(address,count);
  armNextPC = reg[15].I;
  reg[15].I += 2;
  reg[13].I = temp;
  THUMB_PREFETCH;
  busPrefetchCount=0;
  clockTicks += codeTicksAccess16(armNextPC) + codeTicksAccess16(armNextPC);
}
